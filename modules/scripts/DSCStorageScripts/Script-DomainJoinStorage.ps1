...
$SecurityPrincipalNameArray = @()
if ($SecurityPrincipalName -match ',') {
    $SecurityPrincipalNameArray = $SecurityPrincipalName -split ','
} else {
    $SecurityPrincipalNameArray = @($SecurityPrincipalName)
}
...
if ($IdentityServiceProvider -eq 'ADDS') {
    ...
    if ( $CustomOuPath -eq 'true') {
        Join-AzStorageAccount -ResourceGroupName $StorageAccountRG -StorageAccountName $StorageAccountName -OrganizationalUnitDistinguishedName $OUName -DomainAccountType 'ComputerAccount' -EncryptionType 'AES256' -OverwriteExistingADObject
        Write-Log -Message "Successfully domain joined the storage account $StorageAccountName to custom OU path $OUName"
    } else {
        Join-AzStorageAccount -ResourceGroupName $StorageAccountRG -StorageAccountName $StorageAccountName -OrganizationalUnitName $OUName -DomainAccountType 'ComputerAccount' -EncryptionType 'AES256' -OverwriteExistingADObject
...
Try {
    ...
    Write-Log "ACLs set"
    if ($SecurityPrincipalNameArray -contains 'none' -or $IdentityServiceProvider -eq 'EntraID') {
        ...
        foreach ($SPN in $SecurityPrincipalNameArray) {
            if (-not [string]::IsNullOrWhiteSpace($SPN)) {
                $Group = $DomainName + '\' + $SPN
